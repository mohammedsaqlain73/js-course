//* ======================================
//* Modern JavaScript - EcmaScript 2015
 //* =====================================

//* ======================================
//* LET AND CONST - EcmaScript 2015
//* =====================================

//? let Keyword:  The let keyword is used to declare variables with block scope.
//  Variables declared with let are mutable, meaning their values can be reassigned.

 var myFavWorldBestName = "vinod Bahadur Thapa";
 myFavWorldBestName = "thapa technical";
 console.log(myFavWorldBestName);

 let myFavWorldBestName1 = "vinod Bahadur Thapa";
 myFavWorldBestName1 = "thapa technical";
 console.log(myFavWorldBestName1);

//? const Keyword: The const keyword is used to declare variables with block scope,
// but once a value is assigned to a const variable, it cannot be reassigned.
// const variables are immutable.

// pi=3.142 * r * r

// const myFavWorldBestName = "vinod Bahadur Thapa";
// // myFavWorldBestName = "thapa technical";
// console.log(myFavWorldBestName);

//    ================ var ===================
// 1.accesible even outside the bllock
/* function example(){
    var x=10;
}
    console.log(x);
*/
//2. return undefined if try to access before it is declared
/* console.log(x1);
   var x1=90;
 */
//3.mutable i.e can be reassign
/*
  var a=2;
  a=-10;
  console.log(a);
*/
//4.can be redeclared
/*
 var a1=10;
 var a1=20;
   console.log(a1);

 */
//    ================ let ===================
// 1.cant accesible  outside the bllock
/* function example(){
    var y=10;
}
    console.log(y);
*/
//2. return REF. ERROR if try to access before it is declared
/* console.log(y1);
   let y1=90;
 */
//3.mutable i.e can be reassign
/*
  let b=2;
  b=-10;
  console.log(b);
*/
//4.cant  be redeclared
/*
 let b1=10;
 let b1=20;
   console.log(b1);

 */
//    ================ const ===================
// 1.cant accesible  outside the bllock
/* function example(){
    const Z=10;
}
    console.log(Z);
*/
//2. return REF. ERROR if try to access before it is declared
/* console.log(z1);
   const z1=90;
 */
//3.immutable i.e cant be reassign
/*
  const a=2;
  c=-10;
  console.log(c);
*/
//4.cant be redeclared
/*
 const c1=10;
 const c1=20;
   console.log(c1);

 */



//* ==========================================
//* TEMPLATE STRINGS - EcmaScript 2015
//* =========================================

//? In ECMAScript 6 (ES6), template strings, also known as template literals,
//  provide a convenient and flexible way to create strings in JavaScript.
//  Template strings are enclosed in backticks `` rather than single or double quotes and $
//syntax-`$ {str1} ${str2}`

 let firstName = "md";
 let lastName = "saqlain";
 let fullname = firstName + lastName;
 console.log(fullname);//output-mdsaqlain no space but by using template string

// // Using template string for string interpolation
 let fullName = `${firstName} ${lastName}`;
 console.log(fullName);//output-md saqlain with space

//? String Interpolation:  Template strings support string interpolation,
//  allowing you to embed directly within the string. Interpolated expressions are enclosed in
//syntax-` ${} `

 let age = 30;

// // String interpolation with variable
let message =" I am age years old.";
let message1 = `I am ${age} years old.`;
console.log(message);
console.log(message1);//now you got the difference
// Multi-line Strings: Template strings make it easy to create multi-line strings without the need for concatenation or escape characters.
//syntax= `strings`
let multiLineString = `
   This is a multi-line
   string using template literals.
   It's much cleaner and easier to read.`;

 console.log(multiLineString);

//? Expression Evaluation: Template expressions can include any valid JavaScript expression.

//! Practice Time
 let num = 5;
 console.log("5 * " + num + " = " + 5 * num);
console.log(`5 * ${num} = ${5 * num}`);



//============ THE $ CAN ALSO PERFORM THE ARITHMATIC OPERATION e.g ${a+b}


// //? Advantages:
//? Readability: Template strings make the code more readable, especially for complex string constructions.

//? Conciseness: They eliminate the need for explicit string concatenation and reduce the use of escape characters.

//? Expression Flexibility: Any JavaScript expression can be embedded within ${}.

//? Multi-line Support: Creating multi-line strings is more straightforward.

//* ==========================================
//* DEFAULT Parameters
//* =========================================

//? In ECMAScript 6 (ES6), default parameters were introduced, providing a more concise way to handle function parameter defaults. Default parameters allow you to specify default values for function parameters in the function declaration itself. If a parameter is not provided when the function is called, the default value is used.

//? Default function parameters allow named parameters to be initialized with default values if no value or undefined is passed.

//! Write a function to find sum of two numbers? What if during function call user only passed one argument?

// function sum(a = 20, myCurClass = 10) {
//   return a + b;
// }
// console.log(sum());

//* ==========================================
//*  FAT ARROW FUNCTION      syntax-: (args) => {body}
//* =========================================
// Syntax
// const functionName = (param1, param2, ...) => {
//     // Function body
//     return result; // Optional
//   };



//? In ECMAScript 6 (ES6), arrow functions, also known as fat arrow functions, were introduced as a concise way to write anonymous functions.

// const sum = function (a, b) {
//   let result = `The sum of ${a} and ${b} is ${a + b}.`;
//   console.log(result);
// };

// sum(5, 5);

// const sum = (a, b) =>
// { console.log(`The sum of ${a} and ${b} is ${a + b}.`);
// }
// sum(5, 5);

//! How to convert the same in fat arrow function

// todo NOTES

// //?🚀 1: If the function body consists of a single expression, the braces {} and the return keyword can be omitted.
const sum1 = (a, b) => `The sum of ${a} and ${b} is ${a + b}`;
console.log(sum1(5, 5));

// //? 🚀 2: If there is only one parameter, the parentheses () around the parameter list can be omitted.
// const square = a => `The square of ${a} is  ${a * a}`;
// console.log(square(5));

// //? 🚀 3: If there are no parameters, use an empty set of parentheses ().
// const greet = () => console.log("Plz LIKE SHARE & SUBSCRIBE!");
// greet();
//-------------------------------------------------------------------------------
// special case fat arrow function does not return anything if uses {}
const arr = ["My name", "is vinod", "thapa"];
const test = arr.map((curele) => {
  curele.split(" ");
});
console.log(test);//undefined values as {} uses write return curele.split(" ");

const arr = ["My name", "is vinod", "thapa"];
const test = arr.map((curele) =>
  curele.split(" ")
);
console.log(test);// single statement in body so valid ans as {} has not used







//! ==========================================
//!  Part 2 of Modern JavaScript
//! =========================================

//* =========================================
//*  Object Properties - Modern JavaScript
//* =========================================
 const naam = "saqlain";
 const age1 = 21;

//? traditional way
 //const person = { name: name, age: age };
//? Using shorthand notation for object property when key and value are same
const person = { naam, age1 };
console.log(person);
// Instead of specifying name: name and age: age, you can simply use name and age directly within the object litera
// l, thanks to ES6 shorthand property notation.

//* ==========================================
//*  Destructuring - Modern JavaScript
//* =========================================
//? Destructuring in JavaScript is a way to extract values from arrays or objects and assign them to variables
//  in a concise and readable manner.
//? Use Case: Makes code cleaner and avoids repetitive copying of values.

//* Destructuring Arrays:
// Destructuring is a JavaScript expression that makes it possible to unpack values from arrays, or properties from objects,
//  into distinct variables. That is, we can extract data from arrays and objects and assign them to variables.

//? 1: Extracting specific elements:
const numbers = [10, 20, 30];
const first1 = numbers[0]; // Traditional way
console.log(first1);
 const [first, second, third] = numbers; //destructuring :- variable numbers is  unpacked
 console.log(second);

//? 2: Ignoring elements:
const [, , third1] = numbers;
 console.log(third1);

//! Interview Questions
//! Write a program to swap two variables without using 3rd variable?
let a = 10;
let b = 30;
// a=30, b=10
//? Mostly will do using 3rd var
// let c = b; //c=30
// b = a; // b=10
// a = c; //a=30

//? without using 3rd variable
// [a, b] = [b, a];
// console.log(a, b);

//* Destructuring Objects:
const user = { name2: "Vinod", age2: 30 };
// we will see the real life use of it in our main project of weather app

//? Extracting properties:
 const myName = user.name2; // Traditional way
const { name2, age2 } = user;//modern method but you mandatory same keys
console.log(age2, name2);
//since keynames should be gievn if you change order then no prblm
//const { age2,name2 } = user;
console.log(age2, name2 );
//NOTE- FOR DES.OBJECTS SAEM KEYS SHOULD BE USED.

//? Renaming properties:
 const { name2: fullName1, age2 :saal } = user; // Rename "name" to "fullName"
 console.log(fullName1 ,saal);

//* ==========================================
//*  Spread Operator - Modern JavaScript
//* =========================================
// JavaScript ES6 (ECMAScript 6) introduced the spread operator. The syntax is three dots(...)
//  followed by the array (or iterable*).  syntax-[...iterable literal]
// ==========================================================    USES ==========================================================
//? 1. Copying an array
let fruit = ["Apple", "Orange", "mango", "banana"];
let newFruits = [...fruit];
 console.log(newFruits);

//? 2: Concatenating arrays / Combining arrays
const numbers1 = [1, 2, 3, 4];
const numbers2 = [4, 5, 6];
 const newNumbers = [...numbers1, ...numbers2];
console.log(newNumbers);

//? 3: Adding Elements to existing array
let fruits = ["Apple", "Orange", "mango", "banana"];
//fruits.push("guava", "grapes");
fruits.push(...["guava", "grapes"]);
 console.log(fruits);

//! One more useCases
//? In JavaScript, when you spread a string using the spread syntax (...), it converts the string into an array
//  of its individual characters.

//? Traditional way
// const country = "INDIA";
// console.log(country.split(""));

//? New way of doing it
 const country = "INDIA";
 console.log([...country]);

//* ==========================================
//*  Rest parameters  - Modern JavaScript
//* =========================================
//? The rest parameter syntax allows a function to accept an indefinite number of arguments as an array,
//  providing a more flexible way to work with functions that can accept varying numbers of arguments.

//? traditional way of doing it
// const sum = (a, b, c, d) => {
//   return a + b + c + d;
// };

//? with rest parameters
const sum = (a, b, ...numbers) => {
  //   console.log(typeof numbers);
  return numbers.reduce((accum, curVal) => (accum = accum + curVal), 0);

};

console.log(sum(1, 2, 3, 4));

//TODO NOTE: A function definition can only have one rest parameter, and
// the rest parameter must be the last parameter in the function definition.

// function wrong1(...one, ...wrong) {}
// function wrong2(...wrong, arg2, arg3) {}
